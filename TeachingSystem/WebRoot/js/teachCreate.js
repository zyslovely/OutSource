...
function specialtyChange(obj, val) {
	...
	dwr.engine._execute(_cfg_host + "/dwr/", 'TeachSysTeacherBean', 'getList',
			val, function(list) {
				$('#class_selector').html("");
...
function teachChange(obj, val) {
	_teachId=val;
	dwr.engine._execute(_cfg_host + "/dwr/", 'TeachSysTeacherBean',
			'getCoursePercentTypeDemoByTeachId', val, function(demo) {

				if (demo) {
					_demoId=demo.id;
					$("#teach_demo").text(demo.name);
					$(".coursePercentTypeDemos_ul").css("display", "none");
					$(".coursePercentTypeDemos_property_ul").css("display",
							"none");
					$("#coursePercentTypeDemos_" + demo.id).css("display",
							"block");
					$("#coursePercentTypeDemos_property_" + demo.id).css(
							"display", "block");
				}
			});
};

$("#teachCreate_save").click(
		function() {
			...
			_teachers = $(".coursePercentTypes_biaozhun_selectteacher_"
					+ _demoId);
			_propertiesOpts = $(".coursePercentTypes_shuxing_li_" + _demoId);
			_objectCounts = $(".coursePercentTypes_biaozhun_input_objectCount_"
					+ _demoId);
			...
			for (i = 0; i < _opts.length; i++) {
				...
				var _courseScorePercent = {
					...
					"teacherId" : $(_teachers[i]).val() == undefined ? 0 : $(
							_teachers[i]).val(),
			...
			for (i = 0; i < _propertiesOpts.length; i++) {
				...
				_propertiesTypeOps = $(".coursePercentTypes_shuxing_checkbox_"
						+ _demoId + "_" + _typeId);
				for (j = 0; j < _propertiesTypeOps.length; j++) {
					...
					if (_checked) {
						var _property = {
							"percentType" : _typeId,
							"propertyId" : $(_propertiesTypeOps[j]).attr(
									"data_property_id"),
							"percent" : $(_percents[i]).val()
						};
			...
			dwr.engine._execute(_cfg_host + "/dwr/", 'TeachSysTeacherBean',
					'addNewCourse', _teachId, _courseScorePercents,
					_semesterId, _classId, _properties, desc, submitCBBrand);
